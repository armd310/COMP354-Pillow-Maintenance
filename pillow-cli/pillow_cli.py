...
from image_processors import (
    ResizeProcessor, FilterProcessor, AdjustmentProcessor, CropProcessor,
    RotateProcessor, FlipProcessor, FormatProcessor, ThumbnailProcessor,
    WatermarkProcessor, EffectProcessor, BorderProcessor, VignetteProcessor
)
from image_operations import CollageCreator, ImageCompositor, ContactSheetCreator, BatchProcessor
...
class PillowCLI:
    ...
    def create_collage(self, input_paths, output_path, cols=2, padding=10):
        ...
        print(f"Created collage with {len(images)} images ({(len(images) + cols - 1) // cols}x{cols} grid)")

    def create_composite(self, background_path: str, overlay_path: str, output_path: str, position=(0, 0), opacity=1.0):
    ...
    def create_contact_sheet(self, input_paths: list, output_path: str, sheet_width=1200, margin=10):
        ...
        contact_sheet = contact_sheet_creator.create_contact_sheet(images, sheet_width, margin)
    ...
    def batch_process(self, input_dir: str, output_dir: str, operation: str, **kwargs):
        ...
        return batch_processor.process_directory(input_dir, output_dir, operation, **kwargs)
    ...
    def process_image(self, input_path: str, output_path: str, processor_name: str, **kwargs) -> Image.Image:
    ...
    def resize_image(self, input_path: str, output_path: str, width=None, height=None, maintain_aspect=True):
        """Resize image with optional aspect ratio preservation"""
        return self.process_image(input_path, output_path, 'resize',
    ...
    def apply_filters(self, input_path: str, output_path: str, filter_type: str):
        ...
        return self.process_image(input_path, output_path, 'filter', filter_type=filter_type)

    def adjust_image(self, input_path: str, output_path: str, brightness=1.0, contrast=1.0, saturation=1.0,
                     sharpness=1.0):
        """Adjust image brightness, contrast, saturation, and sharpness"""
        return self.process_image(input_path, output_path, 'adjust',
                                  brightness=brightness, contrast=contrast,
                                  saturation=saturation, sharpness=sharpness)

    def crop_image(self, input_path: str, output_path: str, x: int, y: int, width: int, height: int):
        """Crop image to specified dimensions"""
        return self.process_image(input_path, output_path, 'crop',
                                  x=x, y=y, width=width, height=height)

    def rotate_image(self, input_path: str, output_path: str, angle: float, expand=True):
        """Rotate image by specified angle"""
        return self.process_image(input_path, output_path, 'rotate', angle=angle, expand=expand)
    ...
    def convert_format(self, input_path: str, output_path: str, format_type='PNG'):
        ...
        return self.process_image(input_path, output_path, 'format', format_type=format_type)
    ...
    def add_watermark(self, input_path: str, output_path: str, watermark_text: str, position='bottom-right',
                      opacity=128):
        """Add text watermark to image"""
        return self.process_image(input_path, output_path, 'watermark',
                                  watermark_text=watermark_text, position=position, opacity=opacity)
    ...
    def create_border(self, input_path: str, output_path: str, border_width=10, border_color='black'):
        """Add border around image"""
        return self.process_image(input_path, output_path, 'border',
    ...
    def apply_vignette(self, input_path: str, output_path: str, strength=0.5):
        ...
        return self.process_image(input_path, output_path, 'vignette', strength=strength)
...
def main():
    ...
    try:
        match args.command:
            case 'resize':
                cli.resize_image(args.input, args.output, args.width, args.height, maintain_aspect=not args.no_aspect)
